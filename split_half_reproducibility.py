...
def run_split_half_test(df, out_dir, ds_label='Replogle'):
    ...
    if 'Sp' in partial_store:
        ...
        for ps_name in ['PS_real', 'PS_euclid', 'PS_mahal']:
            if ps_name in partial_store:
                ...
                print(f"    >>> Sp advantage over {ps_name} "
                      f"(partial rho difference): {delta:+.4f}")
    ...
    if len(df) >= 16:
        ...
        if diffs and all(d > 0 for d in diffs):
            print("    >>> CONSISTENT: high-Sp has higher cos in ALL mag bins")
        elif diffs:
            n_pos = sum(1 for d in diffs if d > 0)
            print(f"    >>> high-Sp has higher cos in {n_pos}/{len(diffs)} bins")
...
def run_cross_dataset_test(df_norman, df_replogle, out_dir):
    ...
    if 'Sp' in partial_store:
        ...
        for ps_name in ['PS_real', 'PS_euclid', 'PS_mahal']:
            if ps_name in partial_store:
                delta = partial_store['Sp'] - partial_store[ps_name]
                verdict = ("Sp MORE reproducible across datasets"
                           if delta > 0 else
                           f"{ps_name} more reproducible across datasets")
                print(f"    >>> Sp cross-dataset advantage over {ps_name}: "
                      f"{delta:+.4f}  ({verdict})")
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='./shesha-crispr',
