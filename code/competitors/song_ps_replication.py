...
DATASET_CONFIGS = {
    ...
    'adamson': {
        # Frozen main Adamson arm is UPR (pilot kept only as legacy alias)
        'name': 'Adamson 2016 UPR (CRISPRi)',
        'loader': getattr(pt.dt, 'adamson_2016_upr_perturb_seq', None) or pt.dt.adamson_2016_pilot,
        'pert_col': 'condition',
        'ctrl_label': 'control',
...
def run_incremental_upr_analysis(datasets_dict, out_dir):
    ...
    for ds_name, df in sorted(datasets_dict.items()):
        ...
        for ps_col, ps_label in PS_TIERS:
            ...
            print(
                f"    |ρ(Sp | Mp+PS → UPR)|={abs(rho_sp_over_ps):.3f} "
                f"p={p_sp_over_ps:.2e}; "
                f"|ρ(PS | Mp+Sp → UPR)|={abs(rho_ps_over_sp):.3f} "
                f"p={p_ps_over_sp:.2e}"
            )
...
def print_summary(anticorr_df, upr_df):
    ...
    if anticorr_df is not None and len(anticorr_df) > 0:
        ...
        if len(real_rows) > 0 and len(euclid_rows) > 0:
            ...
            print(
                f"\n   |partial ρ| with real PS: {mr:.3f}; "
                f"with Euclidean proxy: {me:.3f}"
            )
    ...
    if upr_df is not None and len(upr_df) > 0:
        ...
        if len(real_upr) > 0:
            ...
            n = int(real_upr['sp_adds_beyond_ps'].sum())
            print(
                f"\n   incremental UPR: Sp adds beyond real PS in "
                f"{n}/{len(real_upr)} datasets"
            )
