...
- Stress extraction generalized: also runs on Norman and other datasets where DDIT3 is present
- Replogle added to ablation datasets
...
BOOTSTRAP_NAN_WARN_THRESHOLD = cfg.BOOTSTRAP_NAN_WARN_THRESHOLD
...
# Stress markers to extract (when available in var_names)
STRESS_MARKERS = ['DDIT3', 'ATF4', 'XBP1', 'HSPA5']


# =============================================================================
# REPLOGLE-SPECIFIC LOADER
# =============================================================================
...
def load_papalexi_2021():
    ...
    try:
        ...
        for base in search_paths:
            ...
            candidates += glob.glob(str(base / "**" / "*.h5mu"), recursive=True)
            for c in candidates:
                if c.endswith('.h5mu'):
                    h5mu_path = c
                    break
    ...
    if 'gene_target' in mdata.obs.columns:
        ...
        n_nt = (adata.obs['gene_target'] == 'NT').sum()
        print(f"    Synced 'gene_target'. Control 'NT' has {n_nt} cells")
    else:
        # Try to find it with prefix matching (mudata may prefix as 'rna:gene_target')
        gt_col = None
        for col in mdata.obs.columns:
            if 'gene_target' in col:
                gt_col = col
                break
        if gt_col:
            adata.obs['gene_target'] = mdata.obs[gt_col].values
            print(f"    Synced '{gt_col}' as gene_target")
        else:
...
# Dataset configuration (now includes Replogle and custom Papalexi loader)
DATASETS = {
    'Norman 2019 (CRISPRa)': pt.dt.norman_2019,
    'Adamson 2016 UPR (CRISPRi)': getattr(pt.dt, 'adamson_2016_upr_perturb_seq', None) or pt.dt.adamson_2016_pilot,
    'Dixit 2016 (CRISPR-KO)': pt.dt.dixit_2016,
    'Papalexi 2021 (CRISPR-KO)': load_papalexi_2021,
    'Replogle 2022 (CRISPRi)': load_replogle_2022,
}

# Manual control keywords per dataset (frozen)
MANUAL_CONTROLS = {
    k: v for k, v in cfg.MANUAL_CONTROLS.items() if k in DATASETS
}
...
def extract_stress_markers(adata, pert_col, ctrl_label, markers=None):
    ...
    ----------
    adata : AnnData
        Normalized, log-transformed data (before HVG subset)
    pert_col : str
    ...
    for marker in markers:
        if marker in adata.var_names:
            ...
            for pert in adata.obs[pert_col].unique():
                ...
                val = adata[mask, marker].X.mean()
                if hasattr(val, 'item'):
                    val = val.item()
                marker_expr[pert] = val
            stress_data[marker] = marker_expr
...
def load_dataset(dataset_name, loader_func, n_pcs=50, seed=320):
    ...
    - Control label is 'control' (non-targeting + chr guides pooled)
    """
    random.seed(seed)
    np.random.seed(seed)
    sc.settings.seed = seed
    ...
    if is_replogle:
        ...
        sc.pp.normalize_total(adata_for_stress, target_sum=1e4)
    ...
    if is_papalexi:
        ...
        try:
            adata_for_stress = adata.copy()
            sc.pp.filter_cells(adata_for_stress, min_genes=100)
            sc.pp.normalize_total(adata_for_stress)
            sc.pp.log1p(adata_for_stress)
            stress_data = extract_stress_markers(adata_for_stress, pert_col, ctrl_label)
    ...
    # Standard handling for non-custom-loader datasets
    adata = raw_data

    if type(adata).__name__ == 'MuData':
        print(f"    - Detected MuData object. Available modalities: {list(adata.mod.keys())}")
        try:
            # Try exact matches first, then prefix matches
            rna_key = None
            for key in adata.mod.keys():
                if key in ('rna', 'gex'):
                    rna_key = key
                    break
            if rna_key is None:
                for key in adata.mod.keys():
                    if key.startswith('rna') or key.startswith('gex'):
                        rna_key = key
                        break
            if rna_key is not None:
                print(f"    - Using modality key: '{rna_key}'")
                adata = adata.mod[rna_key]
            else:
                key = list(adata.mod.keys())[0]
                print(f"    - Falling back to first modality: '{key}'")
                adata = adata.mod[key]
        except Exception as e:
    ...
    # Extract stress markers BEFORE HVG filtering (for all datasets)
    adata_for_stress = adata.copy()
    try:
        sc.pp.calculate_qc_metrics(adata_for_stress, inplace=True)
        sc.pp.filter_cells(adata_for_stress, min_genes=100)
        sc.pp.normalize_total(adata_for_stress)
        sc.pp.log1p(adata_for_stress)
        stress_data = extract_stress_markers(adata_for_stress, pert_col, ctrl_label)
...
def run_main_analysis():
    ...
    all_results_knn = []
    control_scales = {}
    all_stress_data = {}  # {dataset_name: {marker: {pert: value}}}

    print("\n" + "=" * 80)
    print("SHESHA CRISPR ANALYSIS v2 - WITH REPLOGLE 2022")
    ...
    # =========================================================================
    # MERGE STRESS DATA INTO MAIN RESULTS
    # =========================================================================
    for marker in STRESS_MARKERS:
        df[f'stress_{marker}'] = np.nan
        for ds_name, ds_stress in all_stress_data.items():
            if marker in ds_stress:
                for idx, row in df[df['dataset'] == ds_name].iterrows():
                    pert = row['perturbation']
                    if pert in ds_stress[marker]:
                        df.loc[idx, f'stress_{marker}'] = ds_stress[marker][pert]
    ...
    # =========================================================================
    # STRESS MARKER CORRELATION ANALYSIS
    # =========================================================================
    print("\n" + "=" * 80)
    print("STRESS MARKER CORRELATION ANALYSIS")
    print("=" * 80)
    ...
    for marker in STRESS_MARKERS:
        ...
        for ds_name in df['dataset'].unique():
            subset = df[(df['dataset'] == ds_name) & df[col].notna()]
            if len(subset) >= 10:
                ...
                seed_counter += 1
                print(f"{ds_name} | {marker}: rho = {ci['rho']:.3f} [{ci['ci_low']:.3f}, {ci['ci_high']:.3f}], n = {len(subset)}")
                stress_corr_results.append({
                    'dataset': ds_name,
                    'marker': marker,
                    'rho': ci['rho'],
                    'ci_low': ci['ci_low'],
    ...
    if stress_corr_results:
        stress_corr_df = pd.DataFrame(stress_corr_results)
        stress_corr_df.to_csv(os.path.join(OUTPUT_DIR, "stress_marker_correlations.csv"), index=False)
        print(f"\nSaved stress correlations to {OUTPUT_DIR}/stress_marker_correlations.csv")
...
def find_pert_column_ablation(adata):
    ...
    possible_cols = ['condition', 'perturbation_name', 'perturbation', 'gene', 'target', 'guide_id']
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        print("  - stress_marker_correlations.csv")
