...
TEST 1 — NONLINEAR DISCORDANCE RESIDUALS:
...
MODALITY_MAP = dict(cfg.MODALITY_MAP)
# legacy CSV keys still seen in older outputs
MODALITY_MAP.update({
    'Adamson 2016 (CRISPRi)': 'CRISPRi',  # maps to pilot via LEGACY_NAME_MAP
    'Dixit 2016 (CRISPRi)': 'CRISPR-KO',
    'Papalexi 2021 (CRISPR)': 'CRISPR-KO',
})

REPLOGLE_MIN_CELLS = cfg.MIN_CELLS
NORMAN_MIN_CELLS = cfg.MIN_CELLS
DIXIT_MIN_CELLS = cfg.MIN_CELLS
ADAMSON_MIN_CELLS = cfg.MIN_CELLS
PAPALEXI_MIN_CELLS = cfg.MIN_CELLS
...
def main():
    ...
    datasets['Dixit 2016 (CRISPR-KO)'] = df_d
    adata_pca_dict['Dixit 2016 (CRISPR-KO)'] = (
    ...
    datasets['Papalexi 2021 (CRISPR-KO)'] = df_p
    adata_pca_dict['Papalexi 2021 (CRISPR-KO)'] = (
        adata_pca_p, 'gene_target', 'NT')

    # --- Adamson 2016 UPR (frozen main arm) ---
    df_a, adata_pca_a = load_and_process(
        'Adamson 2016 UPR (CRISPRi)',
        getattr(pt.dt, 'adamson_2016_upr_perturb_seq', None) or pt.dt.adamson_2016_pilot,
        pert_col='condition', ctrl_label='control',
        clean_func=clean_adamson, min_cells=ADAMSON_MIN_CELLS,
    )
    datasets['Adamson 2016 UPR (CRISPRi)'] = df_a
    adata_pca_dict['Adamson 2016 UPR (CRISPRi)'] = (
