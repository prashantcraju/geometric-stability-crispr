...
Stratify Norman perturbations by magnitude
...
try:
    ...
    class _Sns:
        ...
        def boxplot(*args, **kwargs):
            raise ImportError("seaborn is required for the quartile boxplot")
    sns = _Sns()
import warnings
warnings.filterwarnings('ignore')

# Same folder as null_model_simulation.csv, then pathway / local archives.
...
OUTPUT_DIR = next(
    (p for p in _CSV_ROOTS if p.exists() and p.name in {"CRISPR", "pathway", "shesha-crispr"}),
    cfg.OUTPUT_DIR,
)
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# =============================================================================
# LOAD (CSV first; live Norman download only with --live)
# =============================================================================

_parser = argparse.ArgumentParser()
_parser.add_argument(
    "--live", action="store_true",
    help="Download Norman via pertpy and recompute Sp/HSPA5 (slow, RAM-heavy).",
)
...
print("=" * 80)
print(f"CSV search roots: {[str(p) for p in _CSV_ROOTS if p.exists()]}")
print(f"Output dir: {OUTPUT_DIR}")


def _alias_cols(frame):
    rename = {}
    lower = {c.lower(): c for c in frame.columns}
    for canon, opts in {
        "Sp": ("sp", "stability", "shesha"),
        "Mp": ("mp", "magnitude", "mag"),
        "mean_HSPA5": ("mean_hspa5", "stress_hspa5", "hspa5"),
        "n_cells": ("n_cells", "ncell"),
        "perturbation": ("perturbation", "gene", "pert", "perturbation_name"),
...
def _norman_only(frame):
    if "dataset" not in frame.columns:
        return frame
    frame = frame.copy()
    frame["dataset"] = frame["dataset"].map(cfg.resolve_dataset_name)
    return frame[frame["dataset"].str.contains("Norman", case=False, na=False)]


def _as_pert_index(frame):
    if "perturbation" in frame.columns:
        return frame.set_index("perturbation")
    return frame


def _usable(frame, need=("Sp", "Mp", "mean_HSPA5"), nmin=8):
    if frame is None or not need.issubset(frame.columns):
        return False
    return len(frame.dropna(subset=list(need))) >= nmin


def _hspa5_col(frame):
    return next(
        (c for c in ("mean_HSPA5", "stress_HSPA5", "hspa5", "HSPA5")
         if c in frame.columns and frame[c].notna().sum() >= 8),
        None,
    )


def _finish(frame, source):
    hcol = _hspa5_col(frame)
    if hcol is None or "Sp" not in frame.columns or "Mp" not in frame.columns:
        return None
    out = frame.rename(columns={hcol: "mean_HSPA5"}).copy()
    if "n_cells" not in out.columns:
        out["n_cells"] = np.nan
    out = out.dropna(subset=["Sp", "Mp", "mean_HSPA5"])
    if len(out) < 8:
        return None
    print(f"Loaded Norman HSPA5+Sp from {source}  (n={len(out)})")
    return out


def _load_from_csv():
    named = [
        "norman_hspa5_quartile_analysis.csv",
    ...
    for name in named:
        ...
        if p is not None and p not in candidates:
            candidates.append(p)
    for p in _glob_csvs("*hspa5*.csv", "*stress*.csv", "*euclidean*.csv"):
        if p.name.startswith("stress_partial") or p.name.startswith("stress_modality"):
            continue
        if p.name.startswith("stress_quadrant"):
            continue
    ...
    for path in candidates:
        try:
            raw = _as_pert_index(_norman_only(_alias_cols(pd.read_csv(path))))
        except Exception as exc:
            print(f"  skip {path.name}: {exc}")
            continue
        if not len(raw):
            continue
        done = _finish(raw, path)
        if done is not None:
            return done
        tables.append((path, raw))

    # Merge Sp/Mp from one table with HSPA5 from another (same CRISPR folder).
    sp_tables = [(p, t) for p, t in tables if {"Sp", "Mp"}.issubset(t.columns)]
    h_tables = [(p, t) for p, t in tables if _hspa5_col(t) is not None]
    for sp_path, sp in sp_tables:
        for h_path, h in h_tables:
            if sp_path == h_path:
                continue
            hcol = _hspa5_col(h)
            merged = sp.copy()
            merged["mean_HSPA5"] = merged.index.astype(str).map(
                h[hcol].groupby(h.index.astype(str)).mean()
            )
            done = _finish(merged, f"{sp_path.name} + {h_path.name}")
            if done is not None:
                return done
    return None
...
if df is None and not _args.live:
    print(
        "(or pathway / shesha-crispr).\n"
        "Looked for norman_hspa5_quartile_analysis.csv, "
        "shesha_crispr_results_euclidean.csv, frozen_sp_scores.csv, "
        "and *hspa5* / *stress* tables.\n"
        "Re-run with --live only if you want to recompute from Norman 2019."
    )
    sys.exit(0)
...
if df is None:
    ...
    print("\n>>> Loading Norman 2019 (CRISPRa) live...")
    adata = _pt_datasets.norman_2019()
    ...
    for pert in valid_perts:
        expr = adata[adata.obs[pert_col] == pert, hspa5_idx].X
        if issparse(expr):
            expr = expr.toarray()
        hspa5_scores[pert] = float(np.mean(expr))
    ctrl_expr = adata[adata.obs[pert_col] == ctrl_label, hspa5_idx].X
    ...
    df = pd.DataFrame({"Sp": pd.Series(stab), "Mp": pd.Series(mag)})
...
if np.isfinite(ctrl_hspa5):
    print(f"    Control mean HSPA5 = {ctrl_hspa5:.4f}")
print(f"    Perturbation HSPA5 range: "
      f"[{df['mean_HSPA5'].min():.4f}, {df['mean_HSPA5'].max():.4f}]")
...
ax_a.set_ylabel('Shesha Coherence', fontsize=11, fontweight='bold')
ax_a.set_title('A. Coherence vs Magnitude\n(colored by HSPA5)', fontsize=12,
               fontweight='bold')
...
if n_sig > 0:
    ...
    print("  -> Directionally consistent with elevated HSPA5 in high-discordance bins.")
