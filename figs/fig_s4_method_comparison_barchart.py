#!/usr/bin/env python3
"""
Supplementary Figure S4 (updated) — Magnitude-Stability Correlation by Distance Metric

Reproduces the grouped bar chart (Spearman rho ± 95% CI for Euclidean,
Whitened, k-NN) across all five datasets, now including Papalexi 2021.
...
main_csv     = DATA_DIR / "crispr_correlations_with_ci.csv"
papalexi_csv = DATA_DIR / "papalexi_method_correlations.csv"

if not main_csv.exists():
    raise FileNotFoundError(
        f"Could not find {main_csv}\n"
        "Run geometric_stability_main_analysis.py first to generate this file."
    )
if not papalexi_csv.exists():
    raise FileNotFoundError(
        f"Could not find {papalexi_csv}\n"
        "Run papalexi_method_comparison.py first to generate this file."
    )

# Main correlations (Adamson, Dixit, Norman, Replogle)
df_main = pd.read_csv(main_csv)
# Drop Papalexi from main CSV if present — the dedicated run takes precedence
df_main = df_main[~df_main['dataset'].str.contains('Papalexi', case=False, na=False)].copy()

# Papalexi correlations
df_pap = pd.read_csv(papalexi_csv)
df_pap['dataset'] = 'Papalexi 2021 (CRISPR)'

# Harmonise method names to Title-Case
for df in (df_main, df_pap):
    df['method'] = df['method'].str.strip().str.replace(
        r'(?i)^euclidean$', 'Euclidean', regex=True).str.replace(
        r'(?i)^whitened$',  'Whitened',  regex=True).str.replace(
        r'(?i)^k.?nn$',     'k-NN',      regex=True)

# Select only the three columns we need from the Papalexi CSV
df_pap = df_pap[['dataset', 'method', 'n', 'rho', 'ci_low', 'ci_high', 'p']].copy()

df_all = pd.concat([df_main, df_pap], ignore_index=True)
...
DATASET_ORDER = [
    ('Adamson 2016 (CRISPRi)',  'Adamson'),
    ('Dixit 2016 (CRISPRi)',    'Dixit'),
    ('Norman 2019 (CRISPRa)',   'Norman'),
    ('Papalexi 2021 (CRISPR)',  'Papalexi'),
...
ax.set_title('Magnitude-Stability Correlation by Distance Metric',
