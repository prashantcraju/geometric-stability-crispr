...
def load_replogle() -> pd.DataFrame:
    ...
    """
    euclid_csv = DATA_DIR / "shesha_crispr_results_euclidean.csv"
    disc_csv   = DATA_DIR / "nonlinear_discordance_comparison.csv"
    ...
    # Filter to Replogle rows when a dataset column is present
    for df in (df_euclid, df_disc):
        if "dataset" in df.columns:
            mask = df["dataset"].str.contains("Replogle", case=False, na=False)
...
def load_norman() -> pd.DataFrame:
    ...
    for df in (df_euclid, df_disc):
        if "dataset" in df.columns:
            mask = df["dataset"].str.contains("Norman", case=False, na=False)
            df.drop(df.index[~mask], inplace=True)
...
def draw_panel(
    ...
    if show_ylabel:
        ax.set_ylabel("Stability $S_p$ (cosine)", fontsize=9)
