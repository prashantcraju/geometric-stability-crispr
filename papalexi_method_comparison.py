...
N_BOOTSTRAP = 10_000
CI_LEVEL = 0.95
MIN_CELLS = 10       # Papalexi has fewer cells per perturbation
N_PCS = 50
KNN_K = 50
LOESS_FRAC = 0.4
REGULARIZATION = 1e-6
...
def bootstrap_partial_correlation_ci(x, y, z, n_bootstrap=N_BOOTSTRAP, seed=SEED):
    ...
    def _partial(x, y, z):
        Z = add_constant(z)
        return spearmanr(OLS(x, Z).fit().resid, sm.OLS(y, Z).fit().resid)
...
for ax, (method, mcol, df_m, xlabel) in zip(axes, METHOD_PANELS):
    ...
    ax.set_ylabel('Shesha Stability (cosine)', fontsize=10, fontweight='bold')
    sns.despine(ax=ax)
...
print("\n" + "=" * 80)
print("SUMMARY — Papalexi 2021 (CRISPR)")
print("=" * 80)
